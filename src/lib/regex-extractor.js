...
export function extractWithRegex(text) {
    ...
    const findEmail = (str) => {
        ...
        for (let part of parts) {
            if (part.includes('@') && part.includes('.')) {
                ...
                // Remove common prefix noise: ce63017010, results, etc.
    ...
    // Compact everything to digits and search for DDD + 8/9 digits
    ...
    // --- NAME EXTRACTION (Multi-Line Header Join) ---
    let lines = normalizedText.split('\n').map(l => l.trim()).filter(l => l.length > 0);
    const potentialHeaderLines = [];

    // Collect first 3-5 lines that look like a main header (all capitalized)
    for (let i = 0; i < Math.min(5, lines.length); i++) {
        let line = lines[i].replace(/^(nome|name|candidato|candidate):\s*/i, '').trim();
        if (line.length < 2) continue;
        if (['@', 'http', '.com', 'telefone', 'email', 'contato'].some(w => line.toLowerCase().includes(w))) break;
        ...
        if (isCapitalized) {
            ...
            // Stop at first non-capitalized line if we already started collecting
            break;
        }
    }
